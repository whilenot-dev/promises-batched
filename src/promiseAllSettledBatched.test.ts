...
import { promiseAllSettledBatched } from "./promiseAllSettledBatched.js";

describe("promiseAllSettledBatched", () => {
  test("to batch in 2", async () => {
    ...
    await promiseAllSettledBatched(batchSize, [
      () => delay(1000, 1),
      () => delay(1000, 2),
      () => delay(1000, 3),
    ]);
  ...
  test("to keep order", async () => {
    ...
    const values = await promiseAllSettledBatched(batchSize, [
      () => delay(1000, 1),
      () => delay(1000, 2),
      () => delay(1000, 3),
    ]);
